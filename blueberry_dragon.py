...
NUMROCKS = 80                             # number of grass objects in the active area
...
class BDGame:
    ...
    NUMROCKS = 80  # number of rock objects in the active area
    NUMSQUIRRELS = 30  # number of squirrels in the active area
    SQUIRRELMINSPEED = 3  # slowest squirrel speed
    SQUIRRELMAXSPEED = 7  # fastest squirrel speed
    DIRCHANGEFREQ = 2  # % chance of direction change per frame
    ...
    def __init__(self):
        ...
        for i in range(10):
            self.rock_objs.append(make_new_grass(self.camera_x, self.camera_y))
    ...
    def _delete_unused_objs(self, obj_list):
        ...
        for i in range(len(obj_list) - 1, -1, -1):
            if is_outside_active_area(self.camera_x, self.camera_y, obj_list[i]):
                del obj_list[i]
    ...
    def run_game(self):
        ...
        while True:
            ...
            while len(rock_objs) < NUMROCKS:
                rock_objs.append(make_new_grass(camera_x, camera_y))
            ...
            for d_obj in dragon_objs:
                d_obj['rect'] = pygame.Rect((d_obj['x'] - camera_x,
                                             d_obj['y'] - camera_y - get_bounce_amount(d_obj['bounce'],
                                                                                       d_obj['bouncerate'],
                                                                                       d_obj['bounceheight']),
                                             d_obj['width'],
                                             d_obj['height']))
            ...
            if not game_over_mode and not (invulnerable_mode and flash_is_on):
                player_obj['rect'] = pygame.Rect((player_obj['x'] - camera_x,
                                                  player_obj['y'] - camera_y - get_bounce_amount(player_obj['bounce'],
                                                                                                 BOUNCERATE,
                                                                                                 BOUNCEHEIGHT),
                                                  player_obj['size'],
                                                  player_obj['size']))
                ...
                if event.type == QUIT:
                    ...
                    if event.key in (K_UP, K_w):
                        ...
                        if player_obj['facing'] != LEFT:
                            player_obj['surface'] = pygame.transform.scale(L_DRAGON_IMG,
                        ...
                        if player_obj['facing'] != RIGHT:
                            player_obj['surface'] = pygame.transform.scale(R_DRAGON_IMG,
                                                                           (player_obj['size'], player_obj['size']))
...
def run_game():
    ...
    rock_objs = []    # stores all the grass objects in the game
    ...
    for i in range(10):
        rock_objs.append(make_new_grass(camera_x, camera_y))
        rock_objs[i]['x'] = random.randint(0, WINWIDTH)
        rock_objs[i]['y'] = random.randint(0, WINHEIGHT)
    ...
    while True:
        ...
        # go through all the objects and see if any need to be deleted.
        for i in range(len(rock_objs) - 1, -1, -1):
            if is_outside_active_area(camera_x, camera_y, rock_objs[i]):
                del rock_objs[i]
        for i in range(len(dragon_objs) - 1, -1, -1):
