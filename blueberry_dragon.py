...
def main():
    ...
    R_SQUIR_IMG = pygame.image.load('./images/blueberry-dragon.png')
...
def runGame():
    ...
    for i in range(10):
        grass_objs.append(makeNewGrass(camerax, cameray))
    ...
    while True: # main game loop
        ...
        for i in range(len(grass_objs) - 1, -1, -1):
            if isOutsideActiveArea(camerax, cameray, grass_objs[i]):
                del grass_objs[i]
        for i in range(len(dragon_objs) - 1, -1, -1):
            if isOutsideActiveArea(camerax, cameray, dragon_objs[i]):
                del dragon_objs[i]
        ...
        while len(dragon_objs) < NUMSQUIRRELS:
            dragon_objs.append(makeNewSquirrel(camerax, cameray))

        # adjust camerax and cameray if beyond the "camera slack"
        playerCenterx = player_obj['x'] + int(player_obj['size'] / 2)
        ...
        if not game_over_mode and not (invulnerable_mode and flashIsOn):
            player_obj['rect'] = pygame.Rect( (player_obj['x'] - camerax,
                                              player_obj['y'] - cameray - getBounceAmount(player_obj['bounce'], BOUNCERATE, BOUNCEHEIGHT),
                                              player_obj['size'],
                                              player_obj['size']) )
        ...
        if not game_over_mode:
            ...
            if (moveLeft or moveRight or moveUp or moveDown) or player_obj['bounce'] != 0:
                player_obj['bounce'] += 1
